...
class UploadService
{
    ...
    public function upload()
    {
        ...
        if ($this->file->getError()) {
            return ['message' => $this->file->getErrorMessage(), 'status' => $this->file->getError()];
        }
        ...
        $info['md5'] = md5_file(storage_path('uploads') . '/' . $savePath);
        $info['sha1'] = sha1_file(storage_path('uploads') . '/' . $savePath);
